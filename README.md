...
| **13 Polymorphism (Ex)** | [01 Aggregators](13PolymorphismEx/01aggregators/) | Reads an aggregation mode (`sum`, `average`, or `min`) and a line of integers; uses polymorphic `StreamAggregator` with derived `SumAggregator`, `AverageAggregator`, and `MinAggregator` to scan the stream and print one aggregated value. |
