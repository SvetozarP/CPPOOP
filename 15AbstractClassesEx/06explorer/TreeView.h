...
std::string getTreeView(const std::vector<std::shared_ptr<FileSystemObject>>& rootObjects) {
    ...
    std::vector<std::shared_ptr<FileSystemObject>> sortedRoots = rootObjects;
    std::sort(sortedRoots.begin(), sortedRoots.end(), [](const std::shared_ptr<FileSystemObject> a, const std::shared_ptr<FileSystemObject> b) {
        return a->getName() < b->getName();
        });
